...
from simplify.almanac.steps import Sow


from .combiners.biographies import Biographies
from .combiners.executive import Executive
from .combiners.judiciary import Judiciary
from .combiners.legislature import Legislature


@timer('External data and source preparation')
@dataclass
class CPSow(Sow):
    """Acquires and/or prepares external data related to other branches
    ...
    def __post_init__ (self):
        super().__post_init__()
        self.menu.localize(instance = self, sections = ['almanac', 'cases',
                                                        'general'])
        self.downloaders = {'executive' : Executive,
    ...
    def prepare(self):
        """Replaces generic prepare method in Sow."""
        for external in self.externals:
            instance = self.options[external](
                    menu = self.menu,
                    inventory = self.inventory,
                    jurisdiction = self.jurisdiction)
            instance.prepare()
            self.techniques.update({external : instance})
            instance.prepare()
        if self.lexis_split:
            instance = self.options[external](
                    menu = self.menu,
                    inventory = self.inventory)
            instance.prepare
            self.techniques.update({'lexis_split' : instance})
        return self

@dataclass
class LexisSplit(Technique):
