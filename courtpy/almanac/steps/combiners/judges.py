...
class Judges(Technique):
    ...
    def __post_init__(self):
        ...
        self.sources = {'judge_exp' : 'judge_name',
    ...
    def clean_panel(self, df, in_col, excess):
        df = (df.pipe(self.no_double_space, in_col = in_col)
                .pipe(self.no_breaks, in_col = in_col))
        junk = '(@@|@ @|@ | @)'
    ...
    def __post_init__(self):
        ...
        if self.stage == 'prep':
            ...
            self.employ_path = os.path.join(self.paths.dicts,
                                            'employ_hist.csv')
            self.circuit_nums = ReSearch(
                    file_path = self.circuit_nums_path,
                    encoding = self.settings['files']['encoding'],
                    out_type = 'int',
                    in_col = 'court',
                    out_col = 'circuit_num')
            self.court_nums = ReSearch(
                    file_path = self.court_nums_path,
                    encoding = self.settings['files']['encoding'],
                    out_type = 'int',
                    in_col = 'court',
                    out_col = 'court_num')
            self.jcs = ReSearch(file_path = self.jcs_export,
                               encoding = self.settings['files']['encoding'],
                               out_type = 'float',
                               in_col = 'nid',
                               out_col = 'jcs')
            self.employ = ReSearch(
