...
import numpy as np
import pandas as pd
from simplify.implements import ReSearch
from simplify.managers import Technique


@dataclass
class Biographies(Technique):
...
#                self.bios_df['senior_status_date'][-4:], ''))
    ...
    def _create_ReSearches(self):
        self.circuit_nums = ReSearch(file_path = self.circuit_nums_path,
                                    encoding = self.encoding,
                                    out_type = 'int',
                                    in_col = 'court',
                                    out_col = 'circuit_num')
        self.court_nums = ReSearch(file_path = self.court_nums_path,
                                  encoding = self.encoding,
                                  out_type = 'int',
                                  in_col = 'court',
                                  out_col = 'court_num')
        self.jcs = ReSearch(file_path = self.jcs_path,
                           encoding = self.encoding,
                           out_type = 'float',
                           in_col = 'nid',
                           out_col = 'jcs')
        self.employ = ReSearch(file_path = self.employ_path,
                              encoding = self.encoding,
                              in_col = 'career')
        return self
...
#                                     sheet_name = self.fjc_main_sheet,
    ...
    def create(self, instance, prefix = ''):
