...
ReMatch aims to simplify and speed up creating lookup tables (pseudo-
dictionaries) for use with pandas dataframes and series. This can particularly
help data scientists munging text data with keywords insteadof natural language
processing. ReMatch also adds functionality by having "knot" matching. "Knots"
(keyword-nots) are terms of exclusion such that their presence in a passed
string negate keyword matches.
...
class ReMatch(object):
    ...
    def __post_init__(self):
        if self.in_col:
            self.in_col = self._complete_col_name(in_column = self.in_col)
        else:
            self.in_col = ''
        """
    ...
    def _complete_col_name(self, in_column = '', out_column = ''):
        if in_column:
    ...
    def match(self, df, in_string = '', in_col = '', in_prefix = '',
        ...
        if in_col:
            self.in_col = self._complete_col_name(in_column = self.in_col)
        if out_col:
            self.out_col = self._complete_col_name(out_column = self.out_col)
